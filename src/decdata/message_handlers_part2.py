...
def handle_video_info_response(node, sender_node, message):
    ...
    if video_id:
        ...
        if video_id not in node.video_catalog:
            ...
            print(
                f"Added video {video_id} to local catalog (from peer {sender_node.id})"
            )
            ...
            try:
                ...
                if youtube_data:
                    ...
                    if youtube_success:
                        print(
                            f"Created YouTube platform data for video {video_id} in local NosVid API"
                        )
                ...
                if nostrmedia_data and nostrmedia_data.get("url"):
                    ...
                    if nostrmedia_success:
                        print(
                            f"Set nostrmedia URL for video {video_id} in local NosVid API"
                        )

                # If the other node has the file and we don't, we could initiate a download
                if has_file and not youtube_downloaded:
                    print(
                        f"Node {sender_node.id} has the file for video {video_id}. Consider downloading it."
                    )
