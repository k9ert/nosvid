...
def get_channel_id_from_handle(api_key, handle):
    ...
    request = youtube.search().list(
        part="snippet", q=f"@{handle}", type="channel", maxResults=5
    )
    response = request.execute()
    print(response)
    if response["items"]:
        ...
        for item in response["items"]:
            ...
            if handle.lower() in channel_title.lower():
                print(
                    f"Found channel ID for @{handle}: {channel_id} (Title: {channel_title})"
                )
                return channel_id
        ...
        print(
            f"Found possible channel ID for @{handle}: {channel_id} (Title: {channel_title})"
        )
        return channel_id
    else:
