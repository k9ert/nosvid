...
from ...utils.filesystem import get_video_dir, get_platform_dir, load_json_file, load_text_file, save_json_file, save_text_file
...
def create_youtube_platform(
    ...
    video = video_result.data
    if not video:
        raise HTTPException(status_code=500, detail=f"Failed to create or retrieve video {video_id}")
