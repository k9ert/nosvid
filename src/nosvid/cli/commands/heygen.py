...
def heygen_command(args):
    ...
    try:
        ...
        if not video_id:
            print(
                "No video ID provided. Finding the oldest video that hasn't been translated..."
            )
        ...
        if heygen_metadata and not args.force:
            if "translations" in heygen_metadata:
                for translation in heygen_metadata["translations"]:
                    if (
                        translation.get("output_language") == language
                        and translation.get("status") == "success"
                    ):
                        print(
                            f"Video already translated to {language} with quality {args.quality}."
                        )
        ...
        # Start translation
        print(
            f"Translating video: {title} ({video_id}) to {language} ({iso_code}) with quality {args.quality}"
        )
        ...
        if "translations" in heygen_metadata:
            for i, translation in enumerate(heygen_metadata["translations"]):
                if (
                    translation.get("output_language") == language
                    or translation.get("language_iso") == iso_code
                ):
        ...
        if args.wait:
            print(
                f"Waiting for translation to complete (timeout: {args.timeout} seconds)..."
            )
            ...
            if not status["success"]:
                print(
                    f"Error during translation: {status.get('error', 'Unknown error')}"
                )
                ...
                for i, translation in enumerate(heygen_metadata["translations"]):
                    if translation.get("video_translate_id") == video_translate_id:
                        ...
                        heygen_metadata["translations"][i]["error"] = status.get(
                            "error", "Unknown error"
                        )
                        heygen_metadata["translations"][i][
                            "checked_at"
            ...
            if status["status"] == "success":
                ...
                if args.download:
                    ...
                    download_success = download_translated_video(
                        url=video_url, output_path=output_file, debug=args.debug
                    )
                ...
                for i, translation in enumerate(heygen_metadata["translations"]):
                    if translation.get("video_translate_id") == video_translate_id:
                        ...
                        heygen_metadata["translations"][i]["url"] = status.get(
                            "url", ""
                        )
                        heygen_metadata["translations"][i][
                            "completed_at"
                ...
                print(
                    f"Translation not completed within timeout. Current status: {status['status']}"
                )
                return 1
        else:
            print("Translation started in the background.")
            print(
                f"Check status later with: ./nosvid heygen-status {video_id} --language {args.language} --quality {args.quality}"
            )
            return 0
...
def heygen_status_command(args):
    ...
    try:
        ...
        if not heygen_metadata or "translations" not in heygen_metadata:
            print(
                f"No HeyGen translations found for video: {video_id} with quality: {args.quality}"
            )
            return 1
        ...
        for t in heygen_metadata["translations"]:
            if (
                t.get("output_language") == language
                or t.get("language_iso") == iso_code
            ):
                translation = t
                break
        ...
        status = check_translation_status(
            video_translate_id=video_translate_id, api_key=api_key, debug=args.debug
        )

        if not status["success"]:
            print(
                f"Error checking translation status: {status.get('error', 'Unknown error')}"
            )
            return 1
                ...
                if status["status"] == "success" and "url" in status:
                    ...
                    if args.download:
                        quality_dir = os.path.join(
                            get_platform_dir(video_dir, "heygen"), args.quality
                        )
                        os.makedirs(quality_dir, exist_ok=True)
                        output_file = os.path.join(quality_dir, f"{iso_code}.mp4")
                        ...
                        print(f"Downloading translated video to: {output_file}")
                        download_success = download_translated_video(
                            url=status["url"], output_path=output_file, debug=args.debug
                        )
...
def register_heygen_parser(subparsers):
    ...
    heygen_parser = subparsers.add_parser(
        "heygen", help="Translate videos using HeyGen"
    )
    ...
    heygen_parser.add_argument(
        "--wait", action="store_true", help="Wait for translation to complete"
    )
    heygen_parser.add_argument(
        "--timeout",
        type=int,
    ...
    heygen_status_parser = subparsers.add_parser(
        "heygen-status", help="Check the status of a HeyGen translation"
    )
    heygen_status_parser.add_argument(
        "video_id", type=str, help="ID of the video to check"
    )
    heygen_status_parser.add_argument(
        "--language",
        type=str,
