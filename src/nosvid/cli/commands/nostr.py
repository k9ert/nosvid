    ...
    try:
        ...
        if not args.video_id:
            ...
            videos, _ = list_videos(
                videos_dir=dirs["videos_dir"], metadata_dir=dirs["metadata_dir"]
            )
        ...
        if "nostrmedia_url" not in metadata:
            ...
            if nostrmedia_result != 0:
                print(
                    "Failed to upload to nostrmedia. Continuing with YouTube URL for embedding."
                )
            else:
                ...
                if os.path.exists(metadata_file):
                    ...
                    if (
                        ...
                        if "url" in nostrmedia_data:
                            metadata["nostrmedia_url"] = nostrmedia_data["url"]
                            print(
                                f"Successfully uploaded to nostrmedia: {metadata['nostrmedia_url']}"
                            )

        # Upload the video to Nostr
        result = upload_to_nostr(
            video_file, metadata, args.private_key, debug=args.debug
        )
...
def register_nostr_parser(subparsers):
    ...
    nostr_parser = subparsers.add_parser(
        "nostr", help="Upload videos to the Nostr network"
    )
    ...
    nostr_parser.add_argument(
        "--debug", action="store_true", help="Enable debug output for Nostr upload"
    )
