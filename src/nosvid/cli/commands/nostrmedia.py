...
from ...platforms.nostrmedia import (
    update_nostrmedia_metadata,
    upload_video_to_nostrmedia,
)
from ...platforms.youtube import find_youtube_video_file
from ...utils.filesystem import get_platform_dir, setup_directory_structure
...
def nostrmedia_command(args):
    ...
    try:
        ...
        if (
            "platforms" not in metadata
            or "youtube" not in metadata["platforms"]
            or not metadata["platforms"]["youtube"].get("downloaded", False)
        ):
        ...
        result = upload_video_to_nostrmedia(
            video_file, args.private_key, debug=args.debug
        )
        ...
        if result["success"]:
            ...
            main_metadata = update_platform_metadata(
                main_metadata, "nostrmedia", nostrmedia_metadata
            )
...
def register_nostrmedia_parser(subparsers):
    ...
    nostrmedia_parser = subparsers.add_parser(
        "nostrmedia", help="Upload videos to nostrmedia.com"
    )
    nostrmedia_parser.add_argument(
        "video_id", type=str, help="ID of the video to upload"
    )
    nostrmedia_parser.add_argument(
        "--private-key",
        type=str,
    ...
    nostrmedia_parser.add_argument(
        "--debug", action="store_true", help="Enable debug output for nostrmedia upload"
    )
