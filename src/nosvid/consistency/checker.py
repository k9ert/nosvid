...
class ConsistencyChecker:
    ...
    def check(self, fix_issues: bool = False) -> Dict[str, Any]:
        ...
        """
        self.logger.info(
            f"Checking metadata consistency in {self.output_dir} for channel {self.channel_title}"
        )
        ...
        video_dirs = [
            d
            for d in os.listdir(self.videos_dir)
        ...
        channel_videos_issues = self._verify_against_channel_videos(
            video_dirs, fix_issues
        )
        ...
        if inconsistencies > 0:
            ...
            for i, issue in enumerate(issues, 1):
                ...
                if issue_type == "missing_metadata":
                    ...
                    self.logger.info(
                        f"{i}. {video_id}: Invalid metadata.json file - {issue['error']}"
                    )
                elif issue_type == "generation_error":
                    self.logger.info(
                        f"{i}. {video_id}: Error generating metadata - {issue['error']}"
                    )
                    ...
                    self.logger.info(
                        f"{i}. {video_id}: Test video directory - {issue['error']}"
                        + (" (fixed)" if issue.get("fixed", False) else "")
                    )
                elif issue_type == "sync_history_only":
                    self.logger.info(
                        f"{i}. {video_id}: In sync_history.json only - {issue['error']}"
                        + (" (fixed)" if issue.get("fixed", False) else "")
                    )
    ...
    def _check_video(
        self, video_dir: str, video_id: str, fix_issues: bool
    ) -> Dict[str, Any]:
        """
        ...
        if not os.path.exists(metadata_file):
            ...
            if fix_issues:
                ...
                try:
                    ...
                    self.logger.error(
                        f"Error creating metadata.json for {video_id}: {e}"
                    )
    ...
    def _check_for_nostr_posts(
        self, video_dir: str, metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if "nostr" not in metadata["platforms"]:
            metadata["platforms"]["nostr"] = {"posts": []}
        # If using old format (not array-based), convert to new format
        elif (
            "posts" not in metadata["platforms"]["nostr"]
            and "event_id" in metadata["platforms"]["nostr"]
        ):
            ...
            post_entry = {
                ...
                "uploaded_at": metadata["platforms"]["nostr"].get(
                    "uploaded_at", datetime.now().isoformat()
                ),
            }

            # Create the posts array with the single entry
            metadata["platforms"]["nostr"] = {"posts": [post_entry]}
        ...
        if (
            "nostr" in metadata["platforms"]
            and "posts" in metadata["platforms"]["nostr"]
        ):
    ...
    def _process_nostr_metadata_files(
        self, nostr_dir: str, metadata: Dict[str, Any]
    ) -> None:
        """
        ...
        for item in os.listdir(nostr_dir):
            ...
            self._process_nostr_metadata_file(
                additional_metadata_file, metadata, item[:-5]
            )  # Remove .json extension

    def _process_nostr_metadata_file(
        self,
        metadata_file: str,
        ...
        try:
            ...
            self.logger.error(
                f"Error processing nostr metadata file {metadata_file}: {e}"
            )

    def _verify_against_channel_videos(
        ...
        for channel_file in channel_videos_files:
            ...
            try:
                ...
                if "videos" in channel_data and isinstance(
                ...
                self.logger.error(
                    f"Error loading channel videos file {channel_file}: {e}"
                )

        self.logger.info(
            f"Found {len(valid_video_ids)} valid video IDs in channel_videos JSON files"
        )
        ...
        if os.path.exists(sync_history_path):
            try:
                ...
                self.logger.info(
                    f"Found {len(sync_history_ids)} video IDs in sync_history.json"
                )
            except Exception as e:
        ...
        if sync_history_only_dirs:
            self.logger.warning(
                f"Found {len(sync_history_only_dirs)} directories in sync_history.json but not in channel_videos:"
            )
        ...
        if invalid_dirs:
            self.logger.warning(
                f"Found {len(invalid_dirs)} invalid video directories (not in channel_videos or sync_history):"
            )
        ...
        if fix_issues:
            ...
            if dirs_to_delete:
                self.logger.info(
                    f"Fixing issues by deleting {len(dirs_to_delete)} test/invalid video directories..."
                )
                fixed_count = 0
                ...
                self.logger.info(
                    f"Successfully deleted {fixed_count} out of {len(dirs_to_delete)} directories"
                )

            # Fix sync_history_only directories by adding them to channel_videos
            if sync_history_only_dirs and channel_id:
                self.logger.info(
                    f"Attempting to fix {len(sync_history_only_dirs)} directories found in sync_history.json..."
                )
                ...
                try:
                    ...
                    if not api_key:
                        self.logger.error(
                            "No YouTube API key found in config. Cannot fix sync_history discrepancies."
                        )
                    else:
                        ...
                        for video_id in sync_history_only_dirs:
                            try:
                                self.logger.info(
                                    f"Fetching metadata for video {video_id} from YouTube API..."
                                )
                                ...
                                if response.get("items"):
                                    ...
                                    if snippet.get("channelId") == channel_id:
                                        ...
                                        if not video_exists:
                                            channel_videos_data["videos"].append(
                                                video_entry
                                            )
                                            ...
                                            fixed_count += 1
                                            self.logger.info(
                                                f"Added video {video_id} to channel_videos_{channel_id}.json"
                                            )
                                    else:
                                        self.logger.warning(
                                            f"Video {video_id} does not belong to channel {channel_id}"
                                        )
                                else:
                                    self.logger.warning(
                                        f"Could not fetch video {video_id} from YouTube API"
                                    )
                            except Exception as e:
                                self.logger.error(
                                    f"Error processing video {video_id}: {e}"
                                )
                        ...
                        if fixed_count > 0:
                            ...
                            channel_videos_data["video_count"] = len(
                                channel_videos_data.get("videos", [])
                            )

                            # Save updated channel_videos file
                            channel_videos_file = os.path.join(
                                metadata_dir, f"channel_videos_{channel_id}.json"
                            )
                            save_json_file(channel_videos_file, channel_videos_data)
                            self.logger.info(
                                f"Successfully added {fixed_count} videos to channel_videos_{channel_id}.json"
                            )

                        if fixed_count < len(sync_history_only_dirs):
                            self.logger.info(
                                f"Could not fix {len(sync_history_only_dirs) - fixed_count} videos. "
                                f"These may require manual intervention."
                            )
                except ImportError:
                    self.logger.error(
                        "Could not import required modules to fix sync_history discrepancies."
                    )
                except Exception as e:
            ...
            if test_dirs or invalid_dirs:
                self.logger.info(
                    "To delete test and invalid directories, run with --fix"
                )
