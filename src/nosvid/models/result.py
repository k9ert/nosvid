...
class Result(Generic[T]):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.data is not None:
            ...
            elif isinstance(self.data, list) and all(hasattr(item, 'to_dict') and callable(getattr(item, 'to_dict')) for item in self.data):
                result['data'] = [item.to_dict() for item in self.data]
            # Handle dictionaries with values that have to_dict method
            elif isinstance(self.data, dict) and all(hasattr(value, 'to_dict') and callable(getattr(value, 'to_dict')) for value in self.data.values()):
                result['data'] = {key: value.to_dict() for key, value in self.data.items()}
            else:
