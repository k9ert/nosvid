...
def check_metadata_consistency(output_dir: str, channel_title: str, fix_issues: bool = False) -> Dict[str, Any]:
    ...
    def check_for_nostr_posts(video_dir: str, main_metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # If using old format (not array-based), convert to new format
        elif 'posts' not in main_metadata['platforms']['nostr'] and 'event_id' in main_metadata['platforms']['nostr']:
            ...
            post_entry = {
                ...
                'uploaded_at': main_metadata['platforms']['nostr'].get('uploaded_at', datetime.now().isoformat())
            }
        ...
        if 'nostr' in main_metadata['platforms'] and 'posts' in main_metadata['platforms']['nostr']:
    ...
    if 'platforms' in fresh:
        if 'platforms' not in existing:
            ...
            if 'nostr' in existing['platforms']:
                ...
                if 'posts' not in existing['platforms']['nostr'] and 'event_id' in existing['platforms']['nostr']:
                    # Create a post entry from the existing data
                    post_entry = {
                        'event_id': existing['platforms']['nostr']['event_id'],
                        'pubkey': existing['platforms']['nostr'].get('pubkey', ''),
                        'nostr_uri': existing['platforms']['nostr'].get('nostr_uri', ''),
                        'links': existing['platforms']['nostr'].get('links', {}),
                        'uploaded_at': existing['platforms']['nostr'].get('uploaded_at', datetime.now().isoformat())
                    }
