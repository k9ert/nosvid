...
def process_video_directory(video_dir: str) -> Tuple[List[str], List[str]]:
    ...
    if os.path.exists(youtube_dir):
        # Find chat files
        chat_files = [f for f in os.listdir(youtube_dir) if f.endswith('.live_chat.json')]
        if chat_files:
        ...
        description_files = [f for f in os.listdir(youtube_dir) if f.endswith('.description')]
        if description_files:
            print(f"Found {len(description_files)} description file(s) for video {video_id}")
            for desc_file in description_files:
                ...
                try:
                    ...
                    print(f"Warning: Could not process description file {desc_file_path}: {e}")
    else:
